  ...
  function generateCart(){
    ...
    Shopify.getCart(function(cart){
      ...
      for (let i = 0; i < forLoop; i++) { 
        ...
        htmlCart += `	<div class="mini-cart-meta">`;
        htmlCart += `		<div class="mb-1"><a href="${product.url}">${product.title}</a></div>`;
        if (product.properties !== null && typeof product.properties === 'object') {
          htmlCart += `<ul class="mini-cart-properties list-unstyled small text-muted mb-2">`;
          for (var prop in product.properties) {
            if (Object.prototype.hasOwnProperty.call(product.properties, prop)) {
              var val = product.properties[prop];
              // Skip empty properties or hidden properties starting with '_'
              if (val !== null && val !== '' && prop.indexOf('_') !== 0) {
                // If it's a file upload, it might contain '/uploads/'
                if (typeof val === 'string' && val.indexOf('/uploads/') > -1) {
                  htmlCart += `<li><span>${prop}: </span><a href="${val}" target="_blank">File</a></li>`;
                } else {
                  htmlCart += `<li><span>${prop}: </span><span>${val}</span></li>`;
                }
              }
            }
          }
